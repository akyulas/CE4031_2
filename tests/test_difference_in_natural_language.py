...
def test_subqueries_are_reflected_correctly():
    ...
    assert success_2 == True
    natural_language_string = find_difference_between_two_query_plans(old_query, old_query_plan, new_query, new_query_plan)
    assert natural_language_string.replace("\n", "") == "Query projections has changed from ['l_orderkey', 'o_orderdate', 'o_shippriority', 'sum(l_extendedprice * (1 - l_discount)) as revenue'] in the old query to ['*'] in the new query.The node with node label 9 of type Seq Scan gets mapped to new node label 0 of type Seq Scan and table filter has changed from (o_orderdate < '1995-03-22'::date) into None.Seq Scan (11,Old) and Hash (10,Old) gets deleted before Hash Join (7,Old).Hash Join (7,Old) gets deleted in between Seq Scan (9,Old) and Nested Loop (6,Old).Index Scan (8,Old) gets deleted before Nested Loop (6,Old).Nested Loop (6,Old), Sort (5,Old), Aggregate (4,Old), Gather Merge (3,Old), Aggregate (2,Old), Sort (1,Old) and Limit (0,Old) gets deleted after Hash Join (7,Old)."
    ######################################################################################################################################################################################
def test_subplans_are_reflected_correctly():
    ...
    assert success_2 == True
    natural_language_string = find_difference_between_two_query_plans(old_query, old_query_plan, new_query, new_query_plan)
    assert natural_language_string.replace("\n", "") == "Query projections has changed from ['count(distinct ps_suppkey) as supplier_cnt', 'p_brand', 'p_size', 'p_type'] in the old query to ['*'] in the new query.The node with node label 7 of type Seq Scan gets mapped to new node label 0 of type Seq Scan, relation name supplier has changed into relation name orders, alias has changed from supplier into orders, table filter has changed from ((s_comment)::text ~~ '%Customer%Complaints%'::text) into None, subplan name has changed from SubPlan 1 into None and output name has changed from supplier into orders.Seq Scan (5,Old) gets deleted in between Seq Scan (7,Old) and Hash Join (4,Old).Seq Scan (8,Old) and Hash (6,Old) gets deleted before Hash Join (4,Old).Hash Join (4,Old), Gather (3,Old), Sort (2,Old), Aggregate (1,Old) and Sort (0,Old) gets deleted after Seq Scan (5,Old)."
